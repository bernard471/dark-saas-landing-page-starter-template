...
import { Bell, FileText, LayoutDashboard, Menu, Search, Settings, Users, Phone, PhoneForwarded, MailWarning, X } from "lucide-react"
...
import { PhoneTrackingTable } from "@/components/phone-tracking-table"
...
export default function PhoneTrackingPage() {
  ...
  const { trackedPhones, refreshPhones } = useTrackedPhones();
  ...
  const handleTrackPhone = async () => {
    try {
      const response = await axios.post("/api/phones/track", { phone: phoneToTrack })
      refreshPhones() // Refresh the email list after tracking
      setPhoneToTrack("") // Clear the input
    } catch (error) {
    ...
    <div className="min-h-screen flex flex-col bg-gray-900 text-gray-100">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-6">
            <h2 className="text-2xl font-semibold mb-6">Phone Number Tracking</h2>
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                <CardTitle className="text-teal-100">Recent Phone Number Tracking</CardTitle>
              </CardHeader>
              <CardContent>
        <PhoneTrackingTable phones={trackedPhones} />
      </CardContent>
            </Card>
          </div>
        </main>
